...
proc vgm_rec_auto_next {} {
	variable stop_watchpoint

	# From MBWave basic driver documentation: 0 = not playing, 255 = playing
	set stop-watchpoint [debug set_watchpoint write_mem 0xda00 {} {vgm::do_next}]
}

proc do_next {} {
	variable play_status $::wp_last_value
	variable active
	
	if { $play_status == 255 } { 
		puts "playing started, so start recording again"
		if {$active} {
			vgm::vgm_rec_start
		} else {
			vgm::vgm_rec Moonsound
		}
	}

	if { $play_status == 0 } { 
		puts " "
		puts "playback stopped, save current recording and end recording"
		vgm::vgm_rec_end
	}
}
...
proc vgm_rec_start {} {
	...
	variable file_name
	set recording_text "VGM recording started, recording data for the following sound chips:"
...
proc vgm_rec_end {} {
	...
	variable directory
       	...
       	# mbwave 
	# set file_name [debug read_block "Main RAM" 0xffc6 0x32]

	# mbwave basic driver
        # set file_name [debug read_block "Main RAM" 0xc0dc 0x32]

	# Dutch Moonsound Veterans
	set file_name "1"
	set file_name_partly [format %s%s%s $directory "/" $file_name]
	# End Dutch Moonsound Veterans
	...
	# Dutch Moonsound Veterans
	set title_block [debug read_block Main\ RAM 0x1728 0x100]
        regexp {(.*)Now playing(.*?)by} $title_block matched sub1 title
        set title [string trim $title]
        #puts "Title: $title"

        set title_block [debug read_block Main\ RAM 0x1728 0x100]
        regexp {by(.*?)Now playing} $title_block matched composer
        set composer [string trim $composer]
        #puts "Composer: $composer"

	exec /usr/local/bin/vgm_sptd $file_name 65535 > /dev/null
        if {[file exists "${file_name_partly}_03.vgm"]==1} {
                set file_name ${file_name_partly}_03.vgm
        } else {
		if {[file exists "${file_name_partly}_02.vgm"]==1} {
			set file_name ${file_name_partly}_02.vgm
		} else {
			set file_name ${file_name_partly}_01.vgm
		}
        }
        exec -- /usr/local/bin/vgm_tag -Title:[subst $title] -Author:[subst $composer] -Game:[subst "Dutch Moonsound Veterans"] -Creator:[subst [exec whoami]] -Year:2015 $file_name

	variable file_number
	if { $file_number<10 } {
		set new_file_name [format %s%s%s%s $directory "/0$file_number " $title ".vgm"]
	} else {
		set new_file_name [format %s%s%s%s $directory "/$file_number " $title ".vgm"]
	}
	incr file_number
	...
	puts $new_file_name
        #exec rm /Users/niek/.openMSX/vgm_recordings/1.vgm
	if {[catch {
		eval exec rm [glob $directory/1_0?.vgm]
	} errorText]} {
		puts "Failed to remove remaining tmp files... no problem, just continue"
	}
	# End Dutch Moonsound Veterans
...
namespace export vgm_rec_auto_next
